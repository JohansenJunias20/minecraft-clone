...
void main()
{
    ...
    for(int i=0;i<pointLightCount;i++){
        ...
        // float shadow=ShadowCalcPointLight(FragPos);
        
        float distance=length(pointLight[i].position-FragPos);
        float attenuation=1./(1.+pointLight[i].linear*distance+pointLight[i].quadric*(distance*distance));
        ...
        result+=(ambient+(1.-shadow)*(diffuse+specular));
